...
use App\Models\BkdProfessor;
use App\Models\User;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class ProdiBkdProfesorController extends Controller
{
    public function index(){
        if (!empty(session('dosen'))) {
            $datas = BkdProfessor::where('dosen_id',session('dosen'))->orderBy('created_at','desc')->get();
        }else {
            $datas = BkdProfessor::orderBy('created_at','desc')->get();
        }
        $dosens = User::where('prodi_id',Auth::user()->prodi_id)->where('akses','dosen')->get();
        return view('operator_prodi/bkd_profesor.index',[
            'dosens'    =>  $dosens,
            'datas'     =>  $datas,
        ]);
    }
    ...
    public function add(){
        ...
        return view('operator_prodi/bkd_profesor.add',[
            'dosen' => $dosen,
        ]);
    }
    ...
    public function post(Request $request){
        ...
        $notification = array(
            'message' => 'Berhasil, data bkd profesor'.$request->nama_dosen. 'berhasil ditambahkan!',
            'alert-type' => 'success'
        );
        return redirect()->route('operator_prodi.bkd_profesor')->with($notification);
    }
    ...
    public function edit($id){
        ...
        $dosen = User::findOrFail(session('dosen'));
        $data = BkdProfessor::where('id',$id)->first();
        return view('operator_prodi/bkd_profesor.edit',[
            'dosen' => $dosen,
            'data' => $data,
        ]);
    }
    ...
    public function update(Request $request, $id){
        ...
        BkdProfessor::where('id',$id)->update([
            'jenis_kegiatan'        =>$request->jenis_kegiatan,
            'bukti_penugasan'       =>$request->bukti_penugasan,
            'sks_beban_kerja'       =>$request->sks_beban_kerja,
            'masa_tugas'            =>$request->masa_tugas,
    ...
    public function delete($id){
        BkdProfessor::where('id',$id)->delete();
        $notification = array(
            'message' => ' data bkd profesor dosen berhasil dihapus!',
            'alert-type' => 'success'
        );
        return redirect()->route('operator_prodi.bkd_profesor')->with($notification);
    }
    ...
    public function pilihDosen(Request $request){
        ...
        return redirect()->route('operator_prodi.bkd_profesor');
    }
}
