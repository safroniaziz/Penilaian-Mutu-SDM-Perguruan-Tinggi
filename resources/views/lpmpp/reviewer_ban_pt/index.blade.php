    ...
    <div class="col-md-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title"><i class="fa fa-calendar-times-o"></i>&nbsp;Reviewer Indikator BAN-PT</h3>
                <div class="pull-right">
                    <a href="{{ route('lpmpp.reviewer_indikator_banpt.add') }}" class="btn btn-primary btn-sm btn-flat"><i class="fa fa-plus"></i>&nbsp; Tambah Data Reviewer</a>
                </div>
            </div>
            <div class="box-body">
                <div class="row">
                    ...
                    <table class="table table-striped table-bordered" id="table" style="width:100%;">
                        ...
                        <tbody>
                            ...
                            @foreach ($reviewers as $reviewer)
                                <tr>
                                    <td> {{ $no++ }} </td>
                                    <td> {{ $reviewer->reviewer->nama_lengkap }} </td>
                                    <td>{{ $reviewer->reviewer->nip }}</td>
                                    <td>{{ $reviewer->reviewer->unit->nama_unit }}</td>
                                    <td style="display:inline-block !important;">
                                        <table>
                                            <tr>
                                                <td>
                                                    <form action="{{ route('lpmpp.reviewer_indikator_banpt.delete',[$reviewer->id]) }}" method="POST">
            ...
            {{--  <div class="modal fade modal-danger" id="modaldelete" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form action=" {{ route('lpmpp.fakultas.delete',[$reviewer->id]) }}" method="POST">
                            {{ csrf_field() }} {{ method_field('DELETE') }}
...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('change','#visibility',function(){
            ...
            $.ajax({
            type :'get',
            url: "{{ url('teacher/reviewer/update_visibility') }}",
